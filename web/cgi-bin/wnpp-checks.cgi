...
rows = dbget("SELECT id, SUBSTRING(title from '^([A-Z]{1,3}):.*') as type, SUBSTRING(title from '^[A-Z]{1,3}:[ ]*([^ ]+)') as source, title FROM bugs WHERE package='wnpp' AND status!='done' and id not in (select id from bugs_merged_with where id > merged_with);")

sources = dbget("SELECT DISTINCT source from sources_uniq where release in ('sid', 'experimental')")
...
qasrcs = dbget("select source from sources_uniq where release in ('sid', 'experimental') and maintainer_email = 'packages@qa.debian.org'")
puts "<ul>"
qasrcs.each do |s|
  src = s['source']
...
bugs = dbget("select * from bugs where id in (#{itpita}) and owner = ''")
puts "<ul>"
bugs.each do |r|
  puts "<li><a href=\"http://bugs.debian.org/#{r['id']}\">##{r['id']}</a>: #{r['title']}</li>"
end
